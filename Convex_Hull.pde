class Convex_Hull {
  ...
  float getArea() {
    ...
    if (points.size() > 3) {
      ...
      PVector line1 = PVector.sub(points.get(0), points.get(1));
      PVector line2 = PVector.sub(points.get(1), points.get(2));
      float angle = PVector.angleBetween(line1, line2);
      area = 0.5 * line1.mag() * line2.mag() * sin(angle);
    }
    return area;
  }
  ...
  float getPerimeter() {
    float perimeter = 0;
    for (int i=0; i<final_edge.size(); i++) {
      PVector line = new PVector(0, 0, 0);
      try {
        line = PVector.sub(final_edge.get(i), final_edge.get(i+1));
      } 
      catch (IndexOutOfBoundsException e) {
        line = PVector.sub(final_edge.get(i), final_edge.get(0));
      }
      perimeter+=line.mag();
    }
    return perimeter;
